...
import { Shield, AlertCircle, Lock, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function ProtectPdfPage() {
  ...
  const { status, progress, queueInfo, error, startJob, reset: resetJob } = useJob({
    onSuccess: (blob) => {
      setResult({ blob, filename: `protected_${file?.name || 'document.pdf'}` });
    },
  });

  const handleProtect = async () => {
    if (!file || !password) return;
    await startJob(() => api.protect(file, password));
  };
      ...
      <div className="pt-2">
             ...
             <div className="premium-card p-10 space-y-8 animate-in slide-in-from-right-8 duration-700">
                ...
                <div className="space-y-4">
                   <div className="relative">
                      <label htmlFor="password" className="absolute -top-3 left-4 px-2 bg-white text-[10px] font-black uppercase tracking-widest text-muted-foreground">User Password</label>
                      <input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password..." className="w-full h-16 px-6 rounded-[20px] bg-secondary/30 border-2 border-border focus:border-amber-500 focus:bg-white outline-none text-lg font-medium transition-all" />
                      <button onClick={() => setShowPassword(!showPassword)} className="absolute right-6 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-amber-600 transition-colors">
                         {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
                      </button>
                   </div>
                   <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 flex gap-4">
                      <div className="text-amber-600 shrink-0"><Shield size={20} /></div>
                      <p className="text-xs text-amber-900/70 font-medium leading-relaxed">Notice: We don&apos;t store your passwords. If you lose this password, the PDF cannot be recovered.</p>
                   </div>
                </div>
             </div>
          </div>
        ...
        {isActive && (
          <div className="py-12 relative">
            ...
            <ProgressBar
              progress={progress}
              label={status === 'queued' ? 'Tasks are Queued' : 'Encrypting Document...'}
              sublabel={status === 'queued' && queueInfo ? `Waiting in line... Position: ${queueInfo.position} / ${queueInfo.length}` : 'Applying AES-256 bit encryption to your file.'}
            />
          </div>
        )}
