...
import { RefreshCw, AlertCircle, RotateCw, RotateCcw, FlipHorizontal, FlipVertical } from 'lucide-react';
...
export default function RotatePdfPage() {
  ...
  const handleRotate = async () => {
    ...
    try {
      ...
      const response = await api.rotate(file, transformation, isFlip ? pageRange : '1-z');
      setProgress(80);
      ...
      console.error(err);
      const message = err.response?.data?.message || 'An error occurred while rotating your PDF.';
      setError(message);
      setStatus('error');
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <div className="pt-2">
             ...
             <div className="premium-card p-10 space-y-8 animate-in slide-in-from-right-8 duration-700">
                ...
                <div className="grid grid-cols-3 gap-4">
                   {[
                      ...
                      <button
                         ...
                         className={`flex flex-col items-center justify-center p-6 rounded-[24px] border-2 transition-all gap-4 ${
                            transformation === option.value 
                            ? 'bg-violet-500/10 border-violet-500 text-violet-700 ring-4 ring-violet-500/5' 
                ...
                {transformation === 'flipH' || transformation === 'flipV' ? (
                      ...
                      <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex gap-4">
                         ...
                         <div className="space-y-1">
                            ...
                            <p className="text-[10px] text-amber-900/70 font-medium leading-relaxed">
                               Mirroring pages requires converting the PDF into high-quality images. The resulting file will be a rasterized PDF.
                            </p>
                         </div>
                      </div>
                      ...
                      <div className="space-y-3">
                         ...
                         <div className="relative">
                            <input 
                               ...
                               placeholder="e.g. 1-2, 5, 1-z"
                               className="w-full h-14 px-6 rounded-[16px] bg-secondary/30 border-2 border-border focus:border-violet-500 focus:bg-white outline-none text-sm font-bold transition-all"
                            />
                            <p className="mt-2 text-[10px] text-muted-foreground font-medium pl-1">
                               Use format: <b>1-3</b> for range, <b>5</b> for single page, or <b>1-z</b> for all.
                            </p>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="p-4 rounded-2xl bg-violet-500/5 border border-violet-500/10 flex gap-4">
                      ...
                      <p className="text-xs text-violet-900/70 font-medium leading-relaxed">
                        Tip: This will apply the same transformation to all pages in your document.
                      </p>
                   </div>
                 )}
             </div>
        ...
        {status === 'processing' && (
          <div className="py-12">
            <ProgressBar progress={progress} label="Applying Transformation..." sublabel={`Processing your document with ${transformation} setting.`} />
          </div>
        )}
